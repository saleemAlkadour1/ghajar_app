...
import 'package:ghajar_app/utils/enums/platform_os.dart';

class AppDeviceInfoService {
  static DeviceInfoPlugin? _deviceInfoPlugin;
  static PlatformOS platformOS = PlatformOS.android;

  AppDeviceInfoService();

  factory AppDeviceInfoService.init() {
    _deviceInfoPlugin = DeviceInfoPlugin();
    return AppDeviceInfoService();
  }

  Future<Map<String, dynamic>> loadDeviceData() async {
    assert(_deviceInfoPlugin != null, 'Device info plugin not loaded');

    var deviceData = <String, dynamic>{};

    try {
      if (_deviceInfoPlugin == null) {
        throw Exception('Device info plugin not loaded');
      }
      platformOS =
          PlatformOS.values.tryFrom(kIsWeb ? 'web' : Platform.operatingSystem);
      switch (platformOS) {
